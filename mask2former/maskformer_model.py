...
class MaskFormer(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        if cfg.MODEL.MASK_FORMER.ERROR_ESTIMATION_ON:
            weight_dict.update({"loss_error_mask": cfg.MODEL.MASK_FORMER.ERROR_ESTIMATION_WEIGHT})
            weight_dict.update({"loss_error_dice": cfg.MODEL.MASK_FORMER.ERROR_ESTIMATION_WEIGHT})
        if cfg.MODEL.MASK_FORMER.MASK_REFINEMENT_ON:
            weight_dict.update({"loss_refined_mask": cfg.MODEL.MASK_FORMER.MASK_REFINEMENT_WEIGHT})
            weight_dict.update({"loss_refined_dice": cfg.MODEL.MASK_FORMER.MASK_REFINEMENT_WEIGHT})
        ...
        losses = ["labels", "masks"]
        if cfg.MODEL.MASK_FORMER.ERROR_ESTIMATION_ON:
            losses.append("mask_errors")
        if cfg.MODEL.MASK_FORMER.MASK_REFINEMENT_ON:
            losses.append("refined_masks")
