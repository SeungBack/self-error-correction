...
class SetCriterion(nn.Module):
    ...
    def loss_refined_masks(self, outputs, targets, indices, num_masks):
        """Compute the losses related to the masks: the focal loss and the dice loss.
        targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        assert "pred_refined_masks" in outputs
    ...
    def loss_mask_errors(self, outputs, targets, indices, num_masks):
        """Compute the losses related to the masks: the focal loss and the dice loss.
        targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        assert "pred_mask_errors" in outputs

        src_idx = self._get_src_permutation_idx(indices)
        tgt_idx = self._get_tgt_permutation_idx(indices)
        src_masks = outputs["pred_mask_errors"]
        src_masks = src_masks[src_idx]
        ...
        # N x 1 x H x W
        src_masks = src_masks[:, None] 
        initial_masks = initial_masks[:, None] # logits -inf ~ inf
        initial_masks = F.interpolate(initial_masks, size=target_masks.shape[-2:], mode='bilinear', align_corners=False)
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks):
        loss_map = {
            ...
            'refined_masks': self.loss_refined_masks,
            'mask_errors': self.loss_mask_errors,
        }
    ...
    def forward(self, outputs, targets):
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    if loss in ["refined_masks", "mask_errors"]:
                        #!TODO: add support for these losses
                        continue
